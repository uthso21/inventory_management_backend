...
import (
	...
	"github.com/uthso21/inventory_management_backend/internal/handler"
...
func (h *MLAgentHandler) ProcessQuery(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodPost {
		handler.SendError(w, http.StatusMethodNotAllowed, "Method not allowed")
		return
	}

	var req entities.MLAgentRequest
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
		handler.SendError(w, http.StatusBadRequest, "Invalid request payload: "+err.Error())
		return
	}

	// Validate required fields
	if req.Query == "" {
		handler.SendError(w, http.StatusBadRequest, "query is required")
		return
	}
	if req.Context.ProductID == "" {
		handler.SendError(w, http.StatusBadRequest, "context.product_id is required")
		return
	}
	...
	if err != nil {
		handler.SendError(w, http.StatusServiceUnavailable, "ML service error: "+err.Error())
		return
	}

	handler.SendSuccess(w, "Analysis complete", resp)
}
...
func (h *MLAgentHandler) DemandForecast(w http.ResponseWriter, r *http.Request) {
	...
	if productCtx.ProductID == "" {
		handler.SendError(w, http.StatusBadRequest, "product_id is required")
		return
	}
	...
	handler.SendSuccess(w, "Demand forecast complete", resp)
}
...
func (h *MLAgentHandler) SmartReorder(w http.ResponseWriter, r *http.Request) {
	...
	handler.SendSuccess(w, "Smart reorder analysis complete", resp)
}
...
func (h *MLAgentHandler) PricelistOptimize(w http.ResponseWriter, r *http.Request) {
	...
	handler.SendSuccess(w, "Pricelist optimization complete", resp)
}
...
func (h *MLAgentHandler) FullAnalysis(w http.ResponseWriter, r *http.Request) {
	...
	handler.SendSuccess(w, "Full analysis complete", resp)
}
...
func (h *MLAgentHandler) HealthCheck(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil || !isHealthy {
		handler.SendJSON(w, http.StatusServiceUnavailable, handler.Response{
			Success: false,
	...
	handler.SendSuccess(w, "ML service is healthy", map[string]interface{}{
		"ml_service": "up",
		"go_backend": "up",
	})
}
